...
Biến môi trường chỉ dùng cho hạ tầng và integration. Nội dung website như
thương hiệu, liên hệ, SEO, mạng xã hội và bản đồ chỉ lưu trong `SiteSetting`
và quản lý tại `/admin/settings`.
...
- Production dùng rate limit PostgreSQL dùng chung giữa các instance. In-memory
  fallback chỉ được phép khi phát triển local mà chưa cấu hình database.
- Public reads dùng Data Cache có tag và thời gian revalidate 60 giây. Khi
  revalidation lỗi, cache gần nhất tiếp tục được phục vụ; cold request không có
  cache sẽ thất bại rõ ràng thay vì trả dữ liệu demo trên production.
- URL ảnh chỉ chấp nhận HTTPS từ Cloudinary hoặc Unsplash; URL social, bản
  đồ và video dùng validation riêng theo từng integration.
- Production upload yêu cầu signed Cloudinary upload preset để enforcement
  format và dung lượng không phụ thuộc dữ liệu do browser khai báo.
