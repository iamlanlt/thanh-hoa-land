...
const imageSchema = z.object({
  url: httpUrlSchema(1000, "URL ảnh không hợp lệ"),
...
export const propertySchema = z.object({
  ...
  sortOrder: z.coerce.number().int().min(0).default(0),
  coverImage: optionalHttpUrlSchema(1000, "URL ảnh không hợp lệ").optional(),
...
export const settingsSchema = z
  .object({
    ...
    logoUrl: optionalHttpUrlSchema(500, "URL logo không hợp lệ"),
    faviconUrl: optionalHttpUrlSchema(500, "URL favicon không hợp lệ"),
    seoTitle: z.string().trim().max(160),
    seoDescription: z.string().trim().max(320),
    ogImageUrl: optionalHttpUrlSchema(500, "URL ảnh SEO không hợp lệ"),
    address: z.string().trim().min(2).max(240),
    workingHours: z.string().trim().max(120),
    mapQuery: z.string().trim().max(240),
    mapEmbedUrl: optionalHttpUrlSchema(1000, "URL bản đồ không hợp lệ"),
