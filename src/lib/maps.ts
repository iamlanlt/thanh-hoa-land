...
export function getGoogleMapsEmbedUrl(
  ...
  if (value && isEmbeddableGoogleMapsUrl(value)) return value;

  const { width = 600, height = 400, zoom = 12 } = options ?? {};
  return `https://maps.google.com/maps?width=${width}&height=${height}&hl=vi&q=${encodeURIComponent(query)}&t=&z=${zoom}&ie=UTF8&iwloc=B&output=embed`;
}
