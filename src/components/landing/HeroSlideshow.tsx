  ...
  useEffect(() => {
    ...
    const startRotation = () => {
      ...
      timer = window.setInterval(() => {
        setActiveIndex((current) => {
          // Don't advance until the current slide has actually finished
          // loading — on slow connections this avoids piling up requests
          // for slides that will be swapped out before they ever render.
    ...
    // Rotating the hero repaints it with a new image, and on slow
    // connections that repaint can itself get recorded as the page's
    // Largest Contentful Paint — pushing that metric out for as long as
    // the slideshow keeps rotating during initial load. Waiting for the
    // browser's own LCP signal before rotating avoids that entirely; a
    // timeout is a fallback for browsers without the LCP API.
