...
function comparePublicPropertyOrder(
  a: Pick<PublicProperty, "featured" | "sortOrder" | "publishedAt">,
  b: Pick<PublicProperty, "featured" | "sortOrder" | "publishedAt">,
) {
...
async function getDevelopmentProperties() {
  if (process.env.NODE_ENV === "production") {
    throw new Error("Demo properties are unavailable in production.");
  }
  return (await import("@/data/demo-properties")).demoProperties;
}
...
async function readPublishedProperties(filters?: {
  district?: string;
  type?: string;
  featured?: boolean;
  limit?: number;
  ...
  if (!isDatabaseConfigured) {
    const demoProperties = await getDevelopmentProperties();
...
export const listPublishedProperties = unstable_cache(
  readPublishedProperties,
  ["published-properties"],
  { revalidate: 60, tags: ["public-properties"] },
);

async function readPublishedPropertyCount() {
  assertPublicDataAvailable();
  if (!isDatabaseConfigured) {
    const demoProperties = await getDevelopmentProperties();
...
export const countPublishedProperties = unstable_cache(
  readPublishedPropertyCount,
...
export async function listRecentProperties(limit = 5) {
  ...
  if (!isDatabaseConfigured) {
    return (await getDevelopmentProperties()).slice(0, take);
  }
  return prisma.property.findMany({
...
export const getPropertyBySlug = unstable_cache(async (slug: string) => {
  assertPublicDataAvailable();
  if (!isDatabaseConfigured) {
    const demoProperties = await getDevelopmentProperties();
    return (
...
}, ["property-by-slug"], { revalidate: 60, tags: ["public-properties"] });
...
export async function listRelatedProperties(
  ...
  if (!isDatabaseConfigured) {
    ...
    return demoProperties
      ...
      .sort((a, b) => {
        const score = (item: PublicProperty) =>
